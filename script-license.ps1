# Script para modificar StarUML y eliminar la verificación de licencia
# Instalar asar globalmente
Write-Host "Instalando asar..."
npm install -g asar

# Cambiar de directorio a resources
$path = "C:\Program Files\StarUML\resources"
Set-Location $path

# Paso 3: Extraer app.asar
Write-Host "Extrayendo app.asar..."
asar extract app.asar app

# Modificar license-manager.js
$licensePath = Join-Path $path "app\src\engine\license-manager.js"
Write-Host "Modificando license-manager.js..."

(Get-Content $licensePath) -replace 'async\s+checkLicenseValidity\s*\(\)\s*\{[^}]*\}', @'
async checkLicenseValidity() {
    setStatus(this, true);
}
'@ | Set-Content $licensePath

# Reemplazar contenido de diagram-export.js
$diagramPath = Join-Path $path "app\src\diagram-export.js"
Write-Host "Reemplazando diagram-export.js..."
...
Set-Content $diagramPath $contenidoNuevo

# Reempaquetar app.asar
Write-Host "Reempaquetando app.asar..."
asar pack app app.asar

# Eliminar la carpeta app
Write-Host "Eliminando carpeta temporal app..."
